...
$sFileName = 'journalProcess.php';
$sFileRef = 'journalProcess.php';
$title = 'PENYEDIAAN JOURNAL';
...
if ($action == "proses" && is_array($pk) && count($pk) > 0) {
	...
	if (!checkdate($actionMM, $actionDD, $actionYY)) {
		...
		for ($i = 0; $i < count($pk); $i++) {
			$sSQL = "SELECT docNo, unloadValue FROM instruct WHERE instructID = ".tosql($pk[$i], "Text");
			$rsSEQ = $conn->Execute($sSQL);
			
			if ($rsSEQ && !$rsSEQ->EOF) {
				$docNo = getField($rsSEQ, 'docNo') ?? '';
				$journalAmt += (float)(getField($rsSEQ, 'unloadValue') ?? 0);
			}
		}
		...
		$description = 'LBH ATM LP';
		$atmType = 'ATM';
		
		$sSQL = " 	
		INSERT INTO journal 
			(seqNo, journalNo, journalDate, journalAmt, description, atmType, createdDate, createdBy) VALUES 
			(".tosql($seqNo, "Text").",".tosql($journalNo, "Text").",".tosql($actionDate, "Text").",".tosql($journalAmt, "Text").",
			  ".tosql($description, "Text").",".tosql($atmType, "Text").",".tosql(date("Y-m-d H:i:s"), "Text").",".tosql($sessionUserName, "Text").")";
		...
		for ($i = 0; $i < count($pk); $i++) {
			...
			$unloadAmt = dlookup("instruct", "unloadValue", "instructID = ".tosql($pk[$i], "Text"));
			
			$user = $sessionUserName;
			$event = 'Tarikh ('.$action1Date.') | Journal ('.$journalNo.') - Jumlah (RM'.number_format($unloadAmt, 2).') untuk Dokumen No ('.$docNo.') :';
			
			$sSQL = "UPDATE instruct SET journalID = '".$journalID."' WHERE instructID = '".$pk[$i]."'";
...
$sSQL = " 
SELECT A.*, B.journalNo, B.journalDate FROM instruct A  
LEFT JOIN journal B ON A.journalID = B.journalID 
WHERE A.isDeleted = '0' 
AND A.status = '4' 
AND A.instructDate BETWEEN '".$ddmmyyFrom."' AND '".$ddmmyyTo."'";
...
if ($q <> '') {
	...
	$sSQL .= " ORDER BY A.instructDate DESC";
}
...
<div class="container-modern">
	<div class="page-header-modern">
		...
		<p class="page-subtitle">Pengurusan dan pemprosesan journal ATM</p>
	</div>
		...
		<div class="table-container">
			<table class="table-modern">
				<thead>
					<tr>
						<th width="40">BIL</th>
						<th width="180">DOKUMEN NO<br>TARIKH ARAHAN</th>
						<th width="200">NO ATM</th>
						<th class="text-center" width="120">TARIKH UNLOAD</th>
						<th class="text-center" width="120">JUMLAH UNLOAD</th>
...
if ($GetList->RowCount() <> 0) {  
	...
	while (!$GetList->EOF && $cnt <= $pg) {
		$instructID = getField($GetList, 'instructID') ?? '';
		$docNo = getField($GetList, 'docNo') ?? '';
		$instructDate = getField($GetList, 'instructDate') ?? '';
		$atmID = getField($GetList, 'atmID') ?? '';
		$unloadDate = getField($GetList, 'unloadDate') ?? '';
		$unloadValue = getField($GetList, 'unloadValue') ?? 0;
		$approvedDate = getField($GetList, 'approvedDate') ?? '';
		$instructStatus = getField($GetList, 'status') ?? '';
		$journalID = getField($GetList, 'journalID') ?? 0;
		$journalNo = getField($GetList, 'journalNo') ?? '';
		$journalDate = getField($GetList, 'journalDate') ?? '';
		
		$statusDisplay = '';
		if ($instructStatus !== '' && isset($statusList) && isset($statusVal)) {
			$statusIndex = array_search($instructStatus, $statusVal);
		...
		print '
				<tr>
					...
					<td>
						...
						<span class="text-muted">'.todate('d/m/Y', $instructDate).'</span>
					</td>
					<td><span class="text-bold">'.htmlspecialchars($atmNo).'</span></td>
					<td class="text-center">'.todate('d/m/Y', $unloadDate).'</td>
					<td class="text-right text-bold">'.number_format($unloadValue, 2).'</td>
		...
		if ($sessionGroupID == '4' || $sessionGroupID == '3') {
			...
			if ($journalID == 0) {
				print '<input type="checkbox" name="pk[]" value="'.htmlspecialchars($instructID).'" class="checkbox-modern">';
			}
			print '</td>';
		} else {
