...
$_SESSION['report_header']=array("BIL","SYARIKAT","TARIKH KUTIPAN","TERMINAL","JENIS","RM100","RM50","RM20","RM10","RM5","RM2","RM1","AMAUN (RM)"); 
...
if ($GetRpt->RowCount() <> 0) { 
	...
	while (!$GetRpt->EOF) {
		...
		$_SESSION['report_values'][$i][2]=''.todate('d-m-Y',$GetRpt->fields('collectDate')).'';
		...
		$_SESSION['report_values'][$i][9]=''.$GetRpt->fields('qty5').'';
		$_SESSION['report_values'][$i][10]=''.$GetRpt->fields('qty2').'';
		$_SESSION['report_values'][$i][11]=''.$GetRpt->fields('qty1').'';
		$_SESSION['report_values'][$i][12]=''.number_format($GetRpt->fields('totalValue'),2).'';
		$GetRpt->MoveNext();
		$i++;
		$bil++;
	}
...
?>
<script>window.location.href = "exprt_rpt.php?fn=rptBE";</script>
