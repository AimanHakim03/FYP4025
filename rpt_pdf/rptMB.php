...
if ($mm == $mmT AND $yy == $yyT) {
	$title  = 'LAPORAN BULANAN KUTIPAN MENGIKUT TERMINAL '.$statusDesc.' BAGI BULAN/TAHUN :  '.sprintf("%02d",$mm).'/'.$yy;
} else {
	$title  = 'LAPORAN BULANAN KUTIPAN MENGIKUT TERMINAL '.$statusDesc.' BAGI BULAN/TAHUN :  '.sprintf("%02d",$mm).'/'.$yy.' HINGGA '.sprintf("%02d",$mmT).'/'.$yyT;
}
...
if ($GetRpt->RowCount() <> 0) { 
	while (!$GetRpt->EOF) {
		if ($GetRpt->fields('atmNo') <> $tempATM) {
			...
			if ($tempATM <> '') {
				$HTML .= '	
				<tr><td colspan="10"><hr style="border-size:0.5px; border-color:#999999"></td></tr>
		...
		$HTML .= '	
		<tr style="font-size: 7pt;">
			<td valign="top" align="center"style="width: 100px;">'.todate('d/m/Y',$GetRpt->fields('collectDate')).'</td>
			...
			<td align="right" valign="top"  style="width: 40px;">'.$GetRpt->fields('qty5').'&nbsp;</td>
			<td align="right" valign="top"  style="width: 40px;">'.$GetRpt->fields('qty2').'&nbsp;</td>
			<td align="right" valign="top"  style="width: 40px;">'.$GetRpt->fields('qty1').'&nbsp;</td>
