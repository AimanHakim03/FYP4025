...
$sFileName = 'branch.php';
$sFileRef  = 'branchAddUpdate.php';
$title     = 'JABATAN/CAWANGAN';
...
//---	BEGIN :	PREPARE DEPARTMENT TYPE -----------------------------------------------------------------------------------------
$deptTypeList = Array();
$deptTypeVal = Array();
$sSQL = " SELECT DeptType, DeptDesc FROM DepartmentType ORDER BY DeptDesc ";
$GetList = &$connM->Execute($sSQL);
if ($GetList->RowCount() <> 0){
	while (!$GetList->EOF) {
		array_push ($deptTypeList, $GetList->fields('DeptDesc'));
		array_push ($deptTypeVal, $GetList->fields('DeptType'));
		$GetList->MoveNext();
	}
}
//---	END   :	PREPARE DEPARTMENT TYPE -----------------------------------------------------------------------------------------

$sSQL = " SELECT	A.*, B.DeptDesc FROM Departments A LEFT JOIN DepartmentType B  ON A.DeptType = B.DeptType ";
if ($type <> '') $sSQL .= " WHERE A.DeptType = '".$type."' ";
if ($q <> '') {
	$sSQL = "	SELECT 		Q.* FROM (".$sSQL.") Q 
				WHERE		Q.Name	 LIKE ".tosql($q."%","Text")."
				ORDER BY 	Q.Code ";
} else {
	$sSQL .= " ORDER BY A.Code ";
}
$GetList = &$connM->Execute($sSQL);
...
<table border="0" cellspacing="1" cellpadding="3" width="800" align="center">
	<tr>
    ...
    <tr valign="top" class="Label">
		...
		Carian melalui Jabatan/Cawangan <input type="text" name="q" value="" maxlength="100" size="30" class="Data">
	...
	<tr valign="top" class="textFont">
		<td>
			<table width="100%">
				...
				<td class="textFont">
				Jenis
	    		<select name="type" class="Data"  onchange="document.ITRViewResults.submit();"> 
			   		<option value="">- Semua - ';
	for ($i = 0; $i < count($deptTypeList); $i++) {
		print '		<option value="'.$deptTypeVal[$i].'"';
		if ($type == $deptTypeVal[$i]) print 'selected';
		print 		'>'.$deptTypeList[$i];
	}			
	print '		</select>
				</td>
	...
	if ($GetList->RowCount() <> 0) {  
		...
		while (!$GetList->EOF && $cnt <= $pg) {
			...
			print ' <tr>
						...
						<td class="'.$strClass.'" valign="middle">'.$GetList->fields('Code').'</td>
						<td class="'.$strClass.'" valign="middle">'.$GetList->fields('Name').'</td>
		...
		<tr>
			<td>';
				if ($TotalRec > $pg) {
					...
					for ($i=1; $i <= $numPage; $i++) {
						$linkRef = $sFileName.'?StartRec='.(($i * $pg) + 1 - $pg).'&pg='.$pg.'&q='.$q.'&type='.$type;
