...
if ($action == "reset") {
    $pwd = 'abc123';
	for ($i = 0; $i < count($pk); $i++) {
		$groupName = dlookup("users", "userName", "userID=" . tosql($pk[$i], "Text"));
		//--- Begin : Create log 
		$user  = $sessionUserName;
		$event = 'Kata Laluan untuk ID Pengguna ('.$groupName.') - Diresetkan';
		$type  = $sessionGroupName;
		createLog($user , $event, $type);
		//--- End   : Create log 	
		$groupName = strtoupper($groupName);			
		$sSQL 	= "	UPDATE users SET password = " . tosql(strtoupper(md5($pwd.$groupName)), "Text").
				  " WHERE	userID =" . tosql($pk[$i], "Number");
		$rs = $conn->Execute($sSQL);
	}
	print '	<script>
				alert("Kata Laluan telah diresetkan kepada abc123");
			</script>';		
}
...
if ($GetList->RowCount() <> 0) {  
	...
	while (!$GetList->EOF && $cnt <= $pg) {
		...
		if ($GetList->fields('contactNo') <> '') $call .= $GetList->fields('contactNo').' ';
		if ($GetList->fields('mobileNo') <> '') $call .= $GetList->fields('mobileNo');
